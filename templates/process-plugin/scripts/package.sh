...
# Validate identity components before using them in the artifact path.
package_identity=$(node -e '
  ...
  process.stdout.write(`${manifest.id}-${manifest.version}-unix`);
' "$manifest_path")
...
(
  cd "$plugin_root"
  zip -X -q -r "$artifact_path" plugin.json bin README.md README.en.md LICENSE
)
